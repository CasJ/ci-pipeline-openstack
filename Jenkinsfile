...
node('rally_agent') {
    ...
    catchError {
        ...
        rally deployment create --fromenv --name=ci-devstack-temp
...
node('master') {
    ...
    step([$class: 'Mailer', notifyEveryUnstableBuild: true, recipients: 'castulo.martinez@intel.com', sendToIndividuals: false])
